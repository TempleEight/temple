...
class GoServiceGeneratorTest extends FlatSpec with Matchers {
  ...
  it should "generate simple services correctly" in {
    GoServiceGenerator.generate(GoServiceGeneratorTestData.simpleServiceRoot)(File("user/dao", "dao.go")) shouldBe GoServiceGeneratorTestData
      .simpleServiceFiles(File("user/dao", "dao.go"))
  }

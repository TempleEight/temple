...
object GrafanaDashboardGeneratorTestUtils {
  ...
  private def makePanels(panels: String*): String =
    panels.map(panel => StringUtils.indent(panel, 4)).mkString(",")

  private def makeTargets(targets: String*): String =
  ...
  def makePanel(
    ...
    val rawTargets = makeTargets(targets: _*)
    s"""|
       |{
        |  "aliasColors" : {
        |        
  ...
  def makeDashboard(uid: String, title: String, panels: String*): String = {
    val rawPanels = makePanels(panels: _*)
    s"""|{
        |  "annotations" : {
        |    "list" : [
        |    ]

...
object GoServiceGeneratorTestData {
  ...
  val simpleServiceFiles: Map[File, FileContent] = Map(
    File("user", "user.go") -> readFile("src/test/scala/temple/generate/language/service/go/testfiles/user_test.go"),
  )
  ...
  val simpleServiceFilesWithComms: Map[File, FileContent] = Map(
    File("match", "match.go") -> readFile("src/test/scala/temple/generate/language/service/go/testfiles/match_test.go"),
  )
}

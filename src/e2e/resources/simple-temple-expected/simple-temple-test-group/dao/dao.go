...
func (dao *DAO) CreateSimpleTempleTestGroup(input CreateSimpleTempleTestGroupInput) (*SimpleTempleTestGroup, error) {
	row := executeQueryWithRowResponse(dao.DB, "INSERT INTO simple_temple_test_group (id) VALUES ($1) RETURNING id;", input.ID)
...
func (dao *DAO) UpdateSimpleTempleTestGroup(input UpdateSimpleTempleTestGroupInput) (*SimpleTempleTestGroup, error) {
	row := executeQueryWithRowResponse(dao.DB, "UPDATE simple_temple_test_group SET WHERE id = $1 RETURNING id;", input.ID)
